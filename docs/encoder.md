...
4. `visualize_components.py` is optional but handy when you want to inspect the exclusivity graph. It reuses the encoder’s parsing helpers to draw a node for every component and an edge for every same-day exclusion, producing `components_graph.png` (or whichever path you pass via `--out`).
...
| `components_graph.png` | Optional visualization generated by `visualize_components.py` showing conflict edges per (week, day). |
