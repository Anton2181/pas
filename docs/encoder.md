...
4. `visualize_components.py` is optional but handy when you want to inspect the exclusivity graph. It now renders multiple views by default—`*_grid.png` aligns nodes by (week, day), `*_spring.png` is force-directed, and `*_component.png` groups connected components. Pass `--layouts` to limit the variants or `--out-prefix` to change the filename prefix.
