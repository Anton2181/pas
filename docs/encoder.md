...
- `family_registry.csv` – a persistent registry of every canonical family token encountered in this run. The encoder rewrites the file with any new tokens, tracks which component IDs map to each family so you can disambiguate similarly named families, and leaves the alias column blank so you can fill in human-readable names that will be reused in subsequent stats/diagnostics.
...
Every sibling/family token is tracked in `family_registry.csv`. The encoder rewrites the registry each run, adding any new canonical tokens, merging their component IDs into the `Components` column, and then looking up the `Alias` column whenever it writes `stats.txt`, `varmap.json`, or other diagnostics. Leave the alias blank to keep using the canonical name or edit it once to give that family a friendly label—the next encoder run will automatically pick it up. Because the registry is an input to future runs it lives under version control, unlike the other gitignored artifacts.
