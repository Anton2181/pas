...
1. **Tier 1 (Priority families)**
   ...
   - *Priority miss guard* (`W_PRIORITY_MISS`) – per-person selector that fires when someone who could have taken a priority task (top or second tier) receives none. This single weight covers both tiers of priority coverage.

2. **Tier 2 (Non-priority families)**
   - Identical cooldown/repeat/streak ladders with weights `W2_COOLDOWN`, `W2_REPEAT`, `W2_STREAK`.

3. **Tier 3** – *Same-day nudger* (`W_FILL_TO_TWO`) encourages filling a second manual-only task on thin days.
...
5. **Tier 4** – *“Both” fallback* (`W_BOTH_FALLBACK`) and *Deprioritized pairs* (`W_DEPRIORITIZED_PAIR`) penalize using the same person to cover both halves of a flexible pair or a deprioritized unordered pair on the same day.

6. **Tier 5** – *Preferred pair miss* (`W_PREFERRED_PAIR_MISS`) charges for every feasible preferred pair that fails to appear.

7. **Tier 6 (Fairness)** – Convex ladders `W_FAIR_OVERLOAD` / `W_FAIR_UNDERLOAD` keep each person’s task load near the global mean (the defaults now keep `FAIR_MEAN_MULTIPLIER = 1.0` and `FAIR_OVER_START_DELTA = 0`, so the baseline target is the unmodified global average before availability scaling kicks in). Under-load penalties only fire when a person falls far below the mean, while over-load ladders grow quadratically to stop hoarding. (Lines 1662‑1915.)

8. **Priority coverage** – Selectors `priority_coverage_vars_top` and `priority_coverage_vars_second` encourage spreading priority tasks across different people or families (configurable via `PRIORITY_COVERAGE_MODE`). A single weight `W_PRIORITY_MISS` powers both tiers so each eligible person pays at most one miss at their highest available priority. These are *soft incentives*; they do **not** force every eligible dancer to receive a priority task. Instead they reward models that cover more distinct names. (Lines 1451‑1582.)

9. **Two-day softening** – When `SUNDAY_TWO_DAY_SOFT=True` or `TWO_DAY_SOFT_ALL=True`, violating the two-day ban adds selectors at weights `W_SUNDAY_TWO_DAY` / `W_TWO_DAY_SOFT` instead of hard-failing the model. The defaults keep `W_TWO_DAY_SOFT = 2e9` and `W_SUNDAY_TWO_DAY = 1.5e9`, which deliberately outrank the coverage selectors so day-violation penalties stay stronger than the incentive to widen priority coverage. (Lines 1044‑1148.)
...
Not as a hard constraint, but the coverage selectors make it costly to skip them. `priority_coverage_vars_top` and `_second` fire when a person (or person/family) with top/second eligibility never receives a matching task. They all share the `W_PRIORITY_MISS` weight, so each eligible person incurs **one** strong penalty when they receive zero tasks at their highest available priority. The guard remains a soft penalty rather than a ban, so the solver can still leave someone without a priority slot when no feasible assignment exists. Inspect `priority_required_vars` plus the coverage maps in `varmap.json` to see which selectors fired.
...
- **Mid-tier nudges should be 3–5 orders smaller.** Weights for the same-day nudger, deprioritized pairs, “Both” fallback, and preferred-pair misses (`W_FILL_TO_TWO`, `W_DEPRIORITIZED_PAIR`, `W_BOTH_FALLBACK`, `W_PREFERRED_PAIR_MISS`) should sit far below cooldown/repeat penalties so they only break ties between otherwise-equal schedules. A common pattern is `W_FILL_TO_TWO ≈ 10^6`, `W_BOTH_FALLBACK ≈ 10^12`, `W_DEPRIORITIZED_PAIR ≈ 10^12`, `W_PREFERRED_PAIR_MISS ≈ 10^3`, keeping them visible but never ahead of Tier 1/2 ladders. 【F:docs/rules.md†L24-L35】
- **Tier 6 fairness should rival Tier 2 once scaled.** After availability scaling, set `W_FAIR_OVERLOAD`/`W_FAIR_UNDERLOAD` so that exceeding a personalized load target by ~2 tasks costs roughly the same as another non-priority repeat hit. This keeps fairness competitive without making it impossible to staff scarce families. The defaults now pin `FAIR_MEAN_MULTIPLIER = 1.0` and `FAIR_OVER_START_DELTA = 0` so the global mean itself becomes the baseline target before availability multipliers stretch or shrink it. Adjust those knobs only when you intentionally want to bias everyone above/below the global average. 【F:docs/rules.md†L32-L52】
- **Two-day penalties outrank coverage selectors.** Keep `W_TWO_DAY_SOFT` above `W_SUNDAY_TWO_DAY`, and both above the coverage weight `W_PRIORITY_MISS`. The current defaults follow `2e9 > 1.5e9 > 1e9`, ensuring that violating a two-day ban is always costlier than duplicating coverage on a priority task. 【F:docs/rules.md†L32-L52】
- **Priority coverage weight lives between Tier 2 and Tier 6.** Because `W_PRIORITY_MISS` only rewards broader coverage, keep it high enough to discourage duplicate assignments (≈10^7–10^9) but below cooldown/repeat ladders so it yields when a coverage-perfect solution would blow up a higher-tier constraint. 【F:docs/rules.md†L34-L35】
